...
public abstract class HttpFormPostHandler extends HttpBinaryHandler implements HttpFileOperator {
	...
	public HttpFormPostHandler(final String route) {
		this(route, null, null, FilenameValidator.makeAllFilesValidator());
	}
	
	public HttpFormPostHandler(final String route, final Treater<byte[]> readingAdapter, final Treater<byte[]> writingAdapter) {
		this(route, readingAdapter, writingAdapter, FilenameValidator.makeAllFilesValidator());
	}
	
	public HttpFormPostHandler(final String route, final String filter) {
		this(route, null, null, new FilenameValidator(filter));
	}
	
	public HttpFormPostHandler(final String route, final Treater<byte[]> readingAdapter, final Treater<byte[]> writingAdapter, final String filter) {
		this(route, readingAdapter, writingAdapter, new FilenameValidator(filter));
	}
	
	public HttpFormPostHandler(final String route, final FilenameValidator filter) {
		this(route, null, null, filter);
	}
	
	public HttpFormPostHandler(final String route, final Treater<byte[]> readingAdapter, final Treater<byte[]> writingAdapter, final FilenameValidator filter) {
